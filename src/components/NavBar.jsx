...
const NavBar = ({handleClick,nav}) => {
    ...
    return <div className='bg-gray-800 flex-none z-[999] w-full h-[3rem] relative  px-2'>
        ...
        <div className="w-10 h-10 rounded-full overflow-hidden ">
         ...
         <ul className={!nav? ' fixed top-0 duration-300 overflow-auto right-[-100%]': 'fixed overflow-auto sm:hidden group w-1/2 z-40 h-screen flex flex-col items-left  bg-gray-900 duration-300 top-0 right-0 px-1' }>
         <Link  to='/'><li className=' w-full  text-pink-500 rounded-lg py-2 cursor-pointer hover:bg-gradient-to-r from-cyan-600'>Home</li></Link> 
